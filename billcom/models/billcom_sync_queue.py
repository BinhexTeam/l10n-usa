...
class BillcomSyncQueue(models.Model):
    ...
    next_retry_date = fields.Datetime(string="Next Retry Date")
            ...
            try:
                ...
                if record.exists():
                    ...
                    if not billcom_id and hasattr(record, "billcom_id"):
                        billcom_id = getattr(record, "billcom_id")
                    elif not billcom_id and hasattr(record, "billcom"):
                        billcom_id = getattr(record, "billcom")
    ...
    def _sync_odoo_to_billcom(self):
        ...
        if not record.exists():
            raise UserError(
                _("Record not found: %s[%s]") % (self.record_model, self.record_id)
            )
    ...
    def _fetch_billcom_data(self):
        ...
        try:
            _logger.info(f"Fetching {self.sync_type} data from BILL API: {self.billcom_id}")
