...
class BillcomFundingAccount(models.Model):
    ...
    is_default_payables = fields.Boolean(
        ...
        readonly=True,
        help="This is the default funding account for payables (vendor payments) in Bill.com",
    )
    is_default_receivables = fields.Boolean(
        string="Default for Receivables",
        readonly=True,
        help="This is the default funding account for receivables (customer invoices) in Bill.com",
    )
    ...
    def sync_funding_accounts_from_billcom(self):
        ...
        try:
            ...
            for account_data in funding_accounts_data:
                ...
                try:
                    ...
                    if existing:
                        existing.write(vals)
                        updated += 1
                        _logger.info(f"Updated funding account: {vals.get('bank_name')}")
                    else:
                        self.create(vals)
                        created += 1
                        _logger.info(f"Created funding account: {vals.get('bank_name')}")
            ...
            _logger.info(
                f"Funding accounts sync completed: {created} created, {updated} updated, {errors} errors"
            )
            ...
            raise UserError(f"Failed to sync funding accounts from Bill.com: {str(e)}")
