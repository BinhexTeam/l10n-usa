...
class BillcomConfig(models.Model):
    ...
    webhook_subscription_state = fields.Selection(
        ...
        string="Webhook Subscription State",
        default="not_subscribed",
        readonly=True,
    )
    webhook_last_error = fields.Text(
        string="Last Webhook Error",
        readonly=True,
    )
    ...
    total_partners = fields.Integer(
        compute="_compute_dashboard_metrics", string="Total Partners"
    )
    synced_vendors = fields.Integer(
        compute="_compute_dashboard_metrics", string="Synced Vendors"
    )
    synced_customers = fields.Integer(
        compute="_compute_dashboard_metrics", string="Synced Customers"
    )
    pending_partners = fields.Integer(
        compute="_compute_dashboard_metrics", string="Pending Partners"
    )

    total_bills = fields.Integer(
        compute="_compute_dashboard_metrics", string="Total Bills"
    )
    synced_bills = fields.Integer(
        compute="_compute_dashboard_metrics", string="Synced Bills"
    )
    pending_bills = fields.Integer(
        compute="_compute_dashboard_metrics", string="Pending Bills"
    )

    total_payments = fields.Integer(
        compute="_compute_dashboard_metrics", string="Total Payments"
    )
    synced_payments = fields.Integer(
        compute="_compute_dashboard_metrics", string="Synced Payments"
    )
    ...
    queue_processing = fields.Integer(
        compute="_compute_dashboard_metrics", string="Queue Processing"
    )
    queue_completed = fields.Integer(
        compute="_compute_dashboard_metrics", string="Queue Completed"
    )
    queue_failed = fields.Integer(
        compute="_compute_dashboard_metrics", string="Queue Failed"
    )
            ...
            config.kanban_dashboard = json.dumps(
                {
                    ...
                    "last_sync_date": config.last_sync_date_dashboard.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
    ...
    def button_test_webhook(self):
        ...
        try:
            ...
            response = service._make_request(
                f"webhook:subscriptions/{self.webhook_subscription_id}/test",
                method="POST",
                data=test_data,
            )
            ...
            raise UserError(_("Failed to send test webhook: %s") % error_msg)
    ...
    def button_sync_webhook_status(self):
        ...
        try:
            ...
            if our_subscription:
                ...
                status_obj = our_subscription.get("status", {})
                billcom_url = our_subscription.get("notificationUrl", "")
                billcom_status = status_obj  # In case status is the state directly
    ...
    def button_view_all_subscriptions(self):
        ...
        try:
            ...
            raise UserError(_("Failed to get subscriptions: %s") % error_msg)
