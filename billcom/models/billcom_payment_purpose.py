...
class BillcomPaymentPurpose(models.Model):
    ...
    def fetch_payment_purposes_for_config(
        ...
        """Fetch payment purposes from Bill.com for specific country/currency/account combination

        Args:
            country_id: res.country record ID
            currency_id: res.currency record ID
            account_type: Bank account type (NONE, CHECKING, SAVINGS)

        Returns:
            List of created/updated billcom.payment.purpose records
        """
        ...
        try:
            ...
            _logger.info(
                f"Fetching payment purposes from Bill.com for country={country.code}, "
            ...
            # Extract payment purposes from response
            # Response structure: {"internationalPayments": {"paymentPurpose": {...}, "validations": {...}}}
            ...
            if not payment_purpose_config:
                _logger.warning(
                    f"No payment purpose configuration returned for country={country.code}, "
                    f"currency={currency.name}, accountType={account_type}"
                )
            ...
            _logger.info(
                f"Payment purpose config: required={purpose_required}, type={purpose_type}, "
                f"codes_count={len(purpose_codes) if isinstance(purpose_codes, list) else 0}"
            )

            # If type is TEXT, there are no predefined codes
            if purpose_type == "TEXT" or not purpose_codes:
                _logger.info(
                    f"Payment purpose type is TEXT or no codes available for country={country.code}. "
            ...
            for purpose_data in purpose_codes:
                # Codes structure: {"name": "description", "value": "CODE"}
                ...
                if existing:
                    ...
                    _logger.info(
                        f"Updated payment purpose: {purpose_code} - {purpose_desc}"
                    )
                else:
                    # Create new record
