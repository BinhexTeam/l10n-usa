...
class BillcomPartnerMatchingLine(models.TransientModel):
    ...
    is_duplicate_group = fields.Boolean(
        string="Is Duplicate Group",
        default=False,
        help="True if this line represents multiple Bill.com records (same vendor, different currencies)",
    )
    ...
    def action_apply_link(self):
        ...
        Bill.com records → one Odoo partner).
        """
        for line in self:
    ...
    def _validate_parent(self, parent):
        """Validate parent is not a child to prevent recursion

        Args:
            parent (res.partner): Partner to validate as parent

        Returns:
            res.partner: Valid parent (original or its parent if it was a child)
        """
        if not parent:
            return None

        # If the proposed parent is itself a child, use ITS parent instead
        if parent.parent_id:
            _logger.warning(
                f"Partner '{parent.name}' (ID: {parent.id}) is a child of '{parent.parent_id.name}'. "
                f"Using '{parent.parent_id.name}' (ID: {parent.parent_id.id}) as parent to prevent recursion."
            )
    ...
    def _link_duplicate_group_to_existing_partner(self):
        """Link duplicate group to existing partner using parent-child structure

        Creates child partners for each Bill.com ID under the specified parent.
        Bank accounts are created in the parent, not children.

        Example:
        Existing Parent: "ABC Corp" (selected in wizard)
          ├─ New Child: "ABC Corp USD" (billcom_id=ven123)
          ├─ New Child: "ABC Corp EUR" (billcom_id=ven456)
          ├─ res.partner.bank (USD account) ← in parent
          └─ res.partner.bank (EUR account) ← in parent
        """
        self.ensure_one()
        ...
        try:
            ...
            for bc_data in billcom_ids_data:
                ...
                if existing_child:
                    ...
                    if existing_child.parent_id != parent_partner:
                        ...
                        _logger.info(
                            f"Updated existing child {existing_child.name} to use parent {parent_partner.name}"
                        )
            ...
            parent_partner.message_post(
                body=_(
                    ...
                    "<ul>"
                    "<li>Parent: %s</li>"
                    "<li>Total Bill.com IDs: %d</li>"
                    "<li>Children Created: %d (%s)</li>"
                    "<li>Children Existing: %d (%s)</li>"
                    "<li>Currencies: %s</li>"
                    "<li>Partner Type: %s</li>"
                    "<li>Linked via: Partner Matching Wizard</li>"
                    "</ul>"
                )
                % (
                    parent_partner.name,
                    len(billcom_ids_data),
                    len(children_created),
                    ", ".join(children_created) if children_created else "none",
                    len(children_existing),
                    ", ".join(children_existing) if children_existing else "none",
                    ", ".join(currencies_summary),
                    partner_type.title(),
                ),
                message_type="notification",
                subtype_xmlid="mail.mt_note",
            ...
            _logger.info(
                ...
                f"{len(children_created)} children created, {len(children_existing)} already existed"
            )
    ...
    def _create_partner_from_duplicate_group(self):
        ...
        try:
            ...
            for bc_data in billcom_ids_data:
                ...
                if existing:
                    ...
                    if existing.parent_id != parent_partner:
                        ...
                        _logger.info(
                            f"Updated existing child {existing.name} to use parent {parent_partner.name}"
                        )
            ...
            parent_partner.message_post(
                body=_(
                    "<p><strong>Parent-Child Structure Created from Bill.com Duplicate Group</strong></p>"
                    "<ul>"
                    "<li>Parent: %s (ID: %d)</li>"
                    "<li>Children created: %d</li>"
                    "<li>Currencies: %s</li>"
                    "<li>Bill.com IDs: %s</li>"
                    "<li>Partner Type: %s</li>"
                ...
                % (
                    ...
                    len(children_created),
                    ", ".join(currencies_summary),
                    ", ".join([bc["id"] for bc in billcom_ids_data]),
                    partner_type.title(),
                ),
                message_type="notification",
                subtype_xmlid="mail.mt_note",
            ...
            _logger.info(
                ...
                f"Parent: {parent_partner.name}, Children: {len(children_created)}, Currencies: {len(currencies_summary)}"
            )
    ...
    def action_apply_create(self):
        ...
        for line in self:
                ...
                line.write(
                    {
                        "state": "error",
                        "error_message": _(
                            "A partner with Bill.com ID %s already exists in Odoo: %s (ID: %d). "
            ...
            try:
                ...
                billcom_data = service._make_request(endpoint, method="GET")
                _logger.info(
                    f"Fetched complete {partner_type} data from Bill.com for creating new partner: {billcom_data.get('name')}"
                )
                ...
                _logger.info(
                    f"Created new Odoo partner {new_partner.name} (ID: {new_partner.id}) from Bill.com {line.billcom_partner_name}"
                )
                ...
                line.write({"state": "error", "error_message": error_msg})
                _logger.error(
                    f"Error creating partner from Bill.com {line.billcom_partner_name}: {error_msg}"
                )
